...
void FAT_Initialize() {
    header_disk = (FileSystemInfo*) kmalloc( sizeof( FileSystemInfo ));
    buffer_disk = (uint8_t*) kmalloc( 512 );
    cd_root_dir = (FileDirectory*) kmalloc( sizeof( FileDirectory ) );
...
char* FAT_ReadFile(FileDirectory* file) {
    ...
    char* buffer = (char*) kmalloc( ((int)(file->file_size / header_disk->bytes_per_sector) + 1) * 512 );
...
char* FAT_LoadFile(const char* filename) {
    ...
    char* fname = (char*) kmalloc( 8 );
    char* efname = (char*) kmalloc( 3 );
    memset( fname, 0x20, 8 );
    ...
    FileDirectory* target_file = (FileDirectory*) kmalloc( sizeof( FileDirectory ) );
...
read_file:
    ...
    char* reserved_data = (char*) kmalloc( ( int )( size / header_disk->bytes_per_sector + 1 ) * header_disk->bytes_per_sector );
...
FileDirectory* FAT_FindFile(const char* name) {
    FileDirectory* trgt = (FileDirectory*) kmalloc( FILE_DIR_SIZE );
    ...
    while( sn_index < strlen(name) ) {
        ...
        while(name[ sn_index ] != '/' && name[ sn_index ] != '\0') {
            ...
            size_fname++;
            char* tmp_n = (char*) kmalloc(size_fname);
        ...
        char* filename_ = (char*) kmalloc(8);
        char* ext_file = (char*) kmalloc(3);
...
FileDirectory* FAT_GetListDirectory(const char* path) {
    ...
    while( cluster < FILE_CLUSTER_EOF ) {
        ...
        for(int i = 0; i < 16; i++) {
            ...
            if( _list == NULL ) {
                _list = (FileDirectory*) kmalloc( alloc_dir_index * FILE_DIR_SIZE );
                memcpy( _list, dir, FILE_DIR_SIZE);
                alloc_dir_index++;
            } else {
                FileDirectory* l = (FileDirectory*) kmalloc( alloc_dir_index * FILE_DIR_SIZE );
